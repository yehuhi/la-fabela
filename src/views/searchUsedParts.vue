<template>
  <div class="q-pa-md" dir="rtl">
    <p style="text-align: center; font-weight: bold; font-size: 35px; color: #ffce0c"> חלקים משומשים </p>
    <div class="row" style="margin: 0 -10px 0 10px; padding: 0 1px 0 1px; direction: rtl">
      ...
      <q-select class="col-12 col-sm-4" bg-color="yellow-7" outlined
                style="padding: 15px"
                standout="bg-yellow-7"
                v-model="localEditedInputs.model"
                :options="carOptions" behavior="menu" label="דגם"/>
      <q-select class="col-12 col-sm-4" bg-color="yellow-7" outlined
                style="padding: 15px"
                standout="bg-yellow-7"
                v-model="localEditedInputs.selectYear"
                :options="YearOptions" behavior="menu" label="שנה"/>

      <q-select
          @keyup(evt)=""
          class="col-4 col-12"
          bg-color="yellow-7" outlined
          style="padding: 15px"
          standout="bg-yellow-7"
    ...
    <div style="padding: 10px 15px">
      <q-expansion-item
          ...
          @show="startCounting"
          @hide="stopCounting"
          header-class="bg-yellow-7 text-black"
          expand-icon-class="text-black"
      >
        <div class="row" style="padding: 15px 0px 10px 0px; direction: rtl; margin-left: 20px">
    ...
    <div class="row justify-center">
      <q-btn v-if="" type="reset"
             style="background-color: #ffcf00; font-size: 20px;top: 10px; margin-bottom: 20px; width: 150px"
             label="חיפוש"
             @click="search()"/>
    </div>
...
export default {
  ...
  methods: {
    ...
    ...mapMutations('items', ['setFilterItems', 'resetEditedItem']),

    search() {
      debugger
      if (this.localEditedInputs) {
        if (!this.localEditedInputs.manufacturer) {
          this.alert = true;
          return
        }
        debugger
